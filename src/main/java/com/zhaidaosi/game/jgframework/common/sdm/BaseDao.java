...
public abstract class BaseDao extends HibernateDaoSupport implements IBaseDao {
	...
	public List<IBaseModel> findByProperty(String propertyName, Object value) {
		log.debug("finding Object instance with property: " + propertyName + ", value: " + value);
		try {
			String queryString = "from " + this.modelName + " as model where model." + propertyName + "= ?";
	...
	public IBaseModel findById(int id) {
		...
		try {
			IBaseModel instance = (IBaseModel) getHibernateTemplate().get(this.entityPackage + this.modelName, id);
			return instance;
		} catch (RuntimeException re) {
