...
public class Router {
	...
	public static void init() {
		...
		for (Class<?> c : classes) {
			...
			try {
				...
				// 只取Handler前面部分
				String handlerName = className.replace(Boot.SERVER_HANDLER_PACKAGE_PATH + ".", "")
						.replace(classSuffix, "").toLowerCase();
	...
	public static IBaseMessage run(InMessage im, Channel ch) throws Exception {
		...
		if (!BaseString.isEmpty(handlerName)) {
			try {
				if (handlerName.equals(HEART_HANDLERNAME)) {// 心跳
					...
					if (handler == null) {
						...
						log.info("router name：" + handler.getHandlerName());
